...
app.config['MYSQL_DB'] = 'riego_inteligente'
mysql = MySQL(app)
...
#url = 'http://192.168.137.117:8080/video' #carrito
#url = 'http://192.168.245.153:8080/video' #carrito
#url = '.\\20240519_173140.mp4'#20240519_173140.mp4<------------------------------
#'.\\20240519_173228.mp4','.\\20240519_173140.mp4','.\\20240519_173059.mp4','.\\20240502_075656.mp4','.\\20240519_173114.mp4'seco
#ESP32
url = 'http://192.168.207.106:8080/video' #carrito
url2 = 'http://192.168.137.110/cam-hi.jpg ' #camara esp 32
...
def index():
    ...
    data = cur.fetchall()
    return render_template('index copy 5.html', historials = data)
...
def handle_start_analysis():
    ...
    if verificador_riego():
        ...
        while True:
            try:
                with urllib.request.urlopen(url2) as img_response:
                    ...
                    lower_yellow = np.array([140, 100, 100])
                    ...
                    if ball_center is not None:
                        if ball_center[0] < third_width:
                            print("aaaaaaaaaaaaaaaaaaaaaaaaaaa",ball_center,"-",third_width)
                            ...
                            if verificador_de_riego[2] == "no regado" and verificador_de_riego[1] == "regado" and verificador_de_riego[0] == "regado":
                                ...
                                primer_riego = False
                                #threading.Thread(target=obtener_video).start()
                                 # Verificar si la pelota está en el centro del cuadrante izquierdo
                                if ball_center[0] > 105 and ball_center[0] < 120:
                                    obtener_video()
                                    break
                            ...
                            if verificador_de_riego[1] == "no regado" and verificador_de_riego[0] == "regado":
                                ...
                                print(position_message)
                                primer_riego = False
                                 # Verificar si la pelota está en el centro del cuadrante central
                                if ball_center[0] > 305 and ball_center[0] < 320:
                                #threading.Thread(target=obtener_video).start()
                                    obtener_video()
                                    break

                        elif ball_center[0]< 3 * third_width:
                            ...
                            if verificador_de_riego[0]== "no regado":
                                ...
                                print(position_message)
                                primer_riego = False
                                # Verificar si la pelota está en el centro del cuadrante derecho
                                if ball_center[0]> 520:
                                    obtener_video()
                                    break
                        #print(position_message)
                        #socketio.emit('position_message', position_message)
                    else:
                        position_message = 'No se detecta la Camara'
                        #time.sleep(3.1)
                    print(position_message)
        ...
        print("No han pasado 12 horas desde el último riego.")
        socketio.emit('riego_mensaje', {'r_mensaje': 'No han pasado 12 horas desde el último riego.'})
        

#BASE DE DATOS
def add_riego(fecha,hora):
    global verificador_de_riego
    fecha = fecha
    hora = hora
    cur = mysql.connection.cursor()
    
    cur.execute('INSERT INTO historial (fecha,hora) VALUES (%s,%s)',(fecha,hora))
    mysql.connection.commit()
    verificador_de_riego = ["no regado","no regado","no regado"]
    return 'recivido'

def verificador_riego():
    cur = mysql.connection.cursor()
    cur.execute('SELECT fecha, hora FROM historial ORDER BY id DESC LIMIT 1')
    result = cur.fetchone()
    if result:
        ultima_fecha = result[0]
        ultima_hora = result[1]
        ultima_fecha_hora = datetime.strptime(f'{ultima_fecha} {ultima_hora}', '%Y-%m-%d %H:%M')
        
        now = datetime.now()
        
        if now - ultima_fecha_hora >= timedelta(hours=12):
            return True
        else:
            return False
    else:
        # Si no hay registros en la base de datos, asumimos que es el primer riego
        return True
